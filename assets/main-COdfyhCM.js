import{$ as a,w as M,f as ea,M as y,s as C,a as U,c as oa,r as K,N as sa,g as z}from"./widev-Dx3a2SVK.js";/* empty css              */import{g as F,d as k,a as w,b as R,q as $,c as S,w as E,e as ia,f as O,u as ta,s as ra,h as D,i as J,j as na,k as ca,l as G}from"./init-PKQicA5Y.js";function la(){const o=`
...
<div id="recuperarModal" class="modal authModals">
  <div class="modal-content">
    ...
    <div class="separador"><span class="sep-number">${o}</span></div>`},B=(o,c)=>{const p=a("#menu-app"),n=o.filter(s=>H(s.estado)).reduce((s,l)=>((s[l.hoja=+l.hoja||0]||(s[l.hoja]=[])).push(l),s),{}),u=Object.keys(c).map(Number).filter(s=>s>0).sort((s,l)=>s-l);if(!u.length)return p.html('<p class="no-items txc">No hay menús disponibles</p>');p.html(u.map(s=>{const l=(n[s]||[]).sort((h,A)=>+(h.orden||0)-+(A.orden||0)||String(h.titulo||"").localeCompare(String(A.titulo||"")));return ma(s,l,c)}).join(""));const b=p.find("img").toArray();Promise.all(b.map(s=>s.complete?Promise.resolve():new Promise(l=>{s.onload=s.onerror=l}))).then(()=>document.fonts?.ready||Promise.resolve()).then(()=>setTimeout(V,80))};async function W(){const o=/\brefresh=1\b/i.test(location.search)||f.noCache,c=(z(m.C_CARTAS)||[]).filter(n=>H(n.estado)),p=(z(m.C_HOJAS)||[]).filter(n=>H(n.estado)),i=Object.fromEntries(p.map(n=>[n.numero,n]));c.length&&Object.keys(i).length&&!o&&B(c,i);try{const[n,u]=await Promise.all([R($(S(w,m.COL_CARTAS),E("estado","==","activo"),G(m.LIM_CARTAS))),R($(S(w,m.COL_HOJAS),E("estado","==","activo"),G(m.LIM_HOJAS)))]),b=n.docs.map(h=>({id:h.id,...h.data()})),s=u.docs.map(h=>({id:h.id,...h.data()})),l=Object.fromEntries(s.map(h=>[h.numero,h]));C(m.C_CARTAS,b,m.HRS),C(m.C_HOJAS,s,m.HRS),B(b,l)}catch(n){console.error("Error cargando menú:",n),c.length||a("#menu-app").html(`
...
Teléfono: ${u.telefono||"-"}
Pedido: ${u.pedido||"-"}
Detalle: ${u.detalle}`;window.open(`https://wa.me/${p}?text=${encodeURIComponent(b)}`,"_blank"),sa("Abriendo WhatsApp...","success",2200),i.currentTarget.reset(),c(a("#foo-recl"))}),a(".wty").text(new Date().getFullYear())});
