import{x as I,$ as e,y as L,r as M,M as A,t as H,b as $,q as E,c as O,a as u,e as W,l as v,h as y,z as J,N as d,j as _,i as T,d as C,A as x,v as P,B as q,C as B,D as N,E as R}from"./widev-BsixQhRF.js";I(y,async f=>{if(!f)return location.href="/";try{const c=H("wiSmile");if(c)return D(c);const j=(await $(E(O(u,"smiles"),W("usuario","==",f.displayName)))).docs[0]?.data();if(!j)return;v("wiSmile",j,450),D(j)}catch(c){console.error(c)}});e(document).on("click",".bt_salir",async()=>{await L(y);try{localStorage.clear()}catch{}location.href="/"});e(document).on("click",".bt_cargar",()=>{M("hojasdbCache","cartasdbCache","wiSmile"),A("Cache limpiado"),setTimeout(()=>location.reload(),700)});function D(f){e(".app").html(`
  <header class="hd">
  ...
  </footer>`),A(`¡Bienvenido ${f.nombre}!`);const c={COL_HOJAS:"hojasdb",COL_CARTAS:"cartasdb",CACHE_HOJAS:"hojasdbCache",CACHE_CARTAS:"cartasdbCache",CACHE_HRS:{hojas:24,cartas:6}},m=[{value:"fa-utensils",title:"Tenedores"},{value:"fa-mug-hot",title:"Café"},{value:"fa-bread-slice",title:"Sandwich"}],j={activo:{value:"activo",icon:"fa-eye",title:"Activo"},desactivo:{value:"desactivo",icon:"fa-eye-slash",title:"Desactivo"}},t={hojas:H(c.CACHE_HOJAS)||[],cartas:H(c.CACHE_CARTAS)||[],hojaActiva:1,platoActivo:null,dirty:{hojas:new Set,cartas:new Set},tempCounter:1},l={fmt:a=>a==null?"0.00":Number(a).toFixed(2),getHoja:a=>t.hojas.find(i=>i.numero===a),getPlatos:a=>t.cartas.filter(i=>i.hoja===a),nextIds:{hoja:()=>`hoja_${Math.max(0,...t.hojas.filter(a=>!a.id.startsWith("temp_")&&/^hoja_/.test(a.id)).map(a=>+a.id.replace("hoja_","")||0))+1}`,carta:()=>`carta_h${String(Math.max(0,...t.cartas.map(a=>+(a.id.match(/carta_h(\d+)/)?.[1]||0)))+1).padStart(2,"0")}`,temp:()=>`temp_${t.tempCounter++}`},iconMeta:a=>m.find(i=>i.value===a)||m[0],nextIcon:a=>m[(m.findIndex(i=>i.value===a)+1+m.length)%m.length].value,estadoMeta:a=>j[a]||j.activo,spin:(a,i,o="Guardando...")=>{i?a.data("o",a.html()).prop("disabled",!0).html(`<i class="fa fa-spinner fa-spin"></i> ${o}`):a.prop("disabled",!1).html(a.data("o")||a.html())}},r={hojas:()=>{const a=e("#hojasList"),i=[...t.hojas].sort((o,s)=>(o.numero||0)-(s.numero||0));if(!i.length)return a.html('<div class="empty-state"><i class="fas fa-layer-group"></i><p>No hay hojas</p></div>');a.html(i.map(o=>{o.icono=o.icono||"fa-utensils",o.estado=o.estado||"activo";const s=l.iconMeta(o.icono),n=l.estadoMeta(o.estado),h=l.getPlatos(o.numero).filter(g=>g.estado==="activo").length;return`
        <div class="hoja-item ${[t.hojaActiva===o.numero?"active":"",t.dirty.hojas.has(o.id)?"dirty":"",o.id.startsWith("temp_")?"temp-item":"",o.estado==="desactivo"?"inactive":""].filter(Boolean).join(" ")}" data-hoja="${o.numero}" data-id="${o.id}">
          <div class="hoja-content">
            ...
            <div class="hoja-info">
              ...
              <p class="nplatos">${h} platos activos ${o.id.startsWith("temp_")?"(sin guardar)":""}</p>
            </div>
          </div>
