...
<div id="recuperarModal" class="modal authModals">
  ...
  `,i=a(".wiAuthCss");i.length?i.text(d):a("head").append(`<style class="wiAuthCss">${d}</style>`)}const l=d=>{const i=a(`#${d}`).addClass("active");a("body").addClass("modal-open"),setTimeout(()=>{i.find("input,select,textarea,button").filter(":visible:first").trigger("focus")},20)},m=d=>{a(`#${d}`).removeClass("active"),a(".modal.active").length||a("body").removeClass("modal-open")},b=()=>{const d=()=>{a(".modal").removeClass("active"),a("body").removeClass("modal-open")};a(document).off(".authModals").on("click.authModals",".close-modal",d).on("click.authModals",".modal",i=>{a(i.target).is(".modal")&&d()}).on("keydown.authModals",i=>{i.key==="Escape"&&d()})};s(),b()}pa();const W=()=>a(".webz").each((e,n)=>a(n).find(".webx").height(a(n).find(".weby").outerHeight())),Z=(()=>{let e;return()=>(clearTimeout(e),e=setTimeout(W,120))})();a(Z);a(window).on("load resize orientationchange",Z);const ma=()=>a(document).off(".wilang").on("click.wilang",".wilang-trigger",e=>{e.stopPropagation();const n=a(e.currentTarget).closest(".midw_v1");n.find(".wilang-content").toggleClass("show"),n.find(".fa-chevron-down").toggleClass("rotated")}).on("click.wilang",".wilang-item",function(e){e.stopPropagation(),a(".wilang-content").removeClass("show"),a(".fa-chevron-down").removeClass("rotated"),setTimeout(()=>location.href=a(this).data("url"),200)}).on("click.wilang",e=>{a(e.target).closest(".midw_v1").length||(a(".wilang-content").removeClass("show"),a(".fa-chevron-down").removeClass("rotated"))});a(()=>ma());const g=window.smile||{},u={COL_CARTAS:g.colCartas||"cartasdb",COL_HOJAS:g.colHojas||"hojasdb",C_CARTAS:g.cacheCartasKey||"cartasdbCachePublic",C_HOJAS:g.cacheHojasKey||"hojasdbCachePublic",HRS:g.cacheHoras||6,LIM_CARTAS:g.limCartas||500,LIM_HOJAS:g.limHojas||50,IMG_FALLBACK:g.imgFallback||"smile.png",ICONO_HOJA:g.iconoHoja||"fa-utensils",CURRENCY:g.currency||"S/",SHOW_ICONS:g.showIcons!==!1},_=e=>String(e||"").toLowerCase()==="activo",ua=e=>e==null||e===""?"":`${u.CURRENCY}${(+e).toFixed(2)}`,fa=e=>`
    <div class="menu-item">
      <div class="item-header">
        <h3>${u.SHOW_ICONS&&e.icono?`<i class="fas ${e.icono}"></i> `:""}${e.titulo||""}</h3>
        <span class="price">${ua(e.precio)}</span>
      </div>
      ${e.descripcion?`<p class="description">${e.descripcion}</p>`:""}
      <div class="dotted-line"></div>
    </div>`,ha=(e,n,f)=>{const s=f[e]||{},l=s.icono||u.ICONO_HOJA,m=s.imagen||u.IMG_FALLBACK,b=s.nota?`<p class="menu-note"><i class="fas fa-info-circle"></i> ${s.nota}</p>`:"";return`
      ...
      <div class="weby">
        <div class="menu-column">
          ...
          <div class="menu-content">
            ${n.length?n.map(fa).join(""):'<p class="no-items">No hay elementos disponibles</p>'}
          </div>
        </div>
      </div>
    </div> 
    <div class="separador"><span class="sep-number">${e}</span></div>`},B=()=>{const e={es:{name:"Español",flag:"https://www.svgrepo.com/show/405610/flag-for-flag-spain.svg",url:"https://cartadehawka.web.app"},en:{name:"English",flag:"https://www.svgrepo.com/show/508668/flag-us.svg",url:"https://coffeehawka.web.app"}},n=Object.entries(e).map(([f,s])=>`<div class="wilang-item" data-lang="${f}" data-url="${s.url}"><img src="${s.flag}" alt="${s.name}" class="flag-icon"><span>${s.name}</span></div>`).join("");return`<header class="hd"><h1 class="wilang"><span class="wilang_titu">Idioma: </span><div class="midw_v1"><div class="wilang-trigger"><div class="wilang-selected"><img src="${e.es.flag}" alt="${e.es.name}" class="flag-icon"><span>${e.es.name}</span></div><i class="fas fa-chevron-down"></i></div><div class="wilang-content">${n}</div></div></h1></header>`},K=()=>`
  <!-- Modales (FOOTER) -->
  ...
  <footer class="foo">
    <div class="foo-inner">
  ...
  </footer>`,V=(e,n)=>{const f=a("#menu-app"),l=e.filter(i=>_(i.estado)).reduce((i,p)=>((i[p.hoja=+p.hoja||0]||(i[p.hoja]=[])).push(p),i),{}),m=Object.keys(n).map(Number).filter(i=>i>0).sort((i,p)=>i-p);if(!m.length)return f.html(B()+'<p class="no-items txc">No hay menús disponibles</p>'+K());const b=m.map(i=>{const p=(l[i]||[]).sort((A,M)=>+(A.orden||0)-+(M.orden||0)||String(A.titulo||"").localeCompare(String(M.titulo||"")));return ha(i,p,n)}).join("");f.html(B()+b+K());const d=f.find("img").toArray();Promise.all(d.map(i=>i.complete?Promise.resolve():new Promise(p=>{i.onload=i.onerror=p}))).then(()=>document.fonts?.ready||Promise.resolve()).then(()=>setTimeout(W,80))};async function Q(){const e=/\brefresh=1\b/i.test(location.search)||g.noCache,n=(U(u.C_CARTAS)||[]).filter(l=>_(l.estado)),f=(U(u.C_HOJAS)||[]).filter(l=>_(l.estado)),s=Object.fromEntries(f.map(l=>[l.numero,l]));n.length&&Object.keys(s).length&&!e&&V(n,s);try{const[l,m]=await Promise.all([E(F(S(w,u.COL_CARTAS),L("estado","==","activo"),G(u.LIM_CARTAS))),E(F(S(w,u.COL_HOJAS),L("estado","==","activo"),G(u.LIM_HOJAS)))]),b=l.docs.map(p=>({id:p.id,...p.data()})),d=m.docs.map(p=>({id:p.id,...p.data()})),i=Object.fromEntries(d.map(p=>[p.numero,p]));C(u.C_CARTAS,b,u.HRS),C(u.C_HOJAS,d,u.HRS),V(b,i)}catch(l){console.error("Error cargando menú:",l),n.length||a("#menu-app").html(`
      ...
      </div>`)}}a(()=>{Q(),a(document).on("keydown",e=>{e.ctrlKey&&e.shiftKey&&e.key.toUpperCase()==="R"&&(Y(u.C_CARTAS,u.C_HOJAS),location.reload())})});window.hawkaMenu={reload:Q,clearCache:()=>Y(u.C_CARTAS,u.C_HOJAS)};a(()=>{const e=s=>a(s).addClass("show").attr("aria-hidden","false"),n=s=>s.removeClass("show").attr("aria-hidden","true"),f=window.smile?.whatsReclamos||"51992472706";a(document).on("click","[data-foomodal]",s=>(s.preventDefault(),e(a(s.currentTarget).data("foomodal")))).on("click",".foo-close,.foomodal .foo-ok,.foomodal .foo-cancel",s=>n(a(s.currentTarget).closest(".foomodal"))).on("click",".foomodal",s=>{s.target===s.currentTarget&&n(a(s.currentTarget))}).on("submit","#reclamosForm",s=>{s.preventDefault();const l=new FormData(s.currentTarget),m=Object.fromEntries(l.entries());if(!m.nombres||!m.email||!m.detalle||!l.get("consent"))return y("Completa los campos obligatorios.","error");const b=`*Reclamo Hawka*
Nombre: ${m.nombres}
Email: ${m.email}
Teléfono: ${m.telefono||"-"}
Pedido: ${m.pedido||"-"}
Detalle: ${m.detalle}`;window.open(`https://wa.me/${f}?text=${encodeURIComponent(b)}`,"_blank"),ta("Abriendo WhatsApp...","success",2200),s.currentTarget.reset(),n(a("#foo-recl"))}),a(".wty").text(new Date().getFullYear())});
